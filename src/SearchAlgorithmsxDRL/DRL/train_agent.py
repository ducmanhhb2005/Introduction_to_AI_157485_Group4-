...
try:
    ...
    from SearchAlgorithmsxDRL.DRL.dqn_agent import DQNAgent
...
MAP_FILE_RELATIVE_TO_PROJECT_ROOT = "InputMap/map11x20.txt" # Tạo file map riêng cho DRL 
# MAP_FILE_RELATIVE_TO_PROJECT_ROOT = "Input/map20x22.txt" # Hoặc dùng map 20x22
...
EPSILON_START = 0.3
EPSILON_END = 0.01
EPSILON_DECAY_RATE = 0.9995 # Giảm 0.5% epsilon sau mỗi episode
BUFFER_CAPACITY = 100000
BATCH_SIZE = 64
MIN_BUFFER_SIZE_TO_LEARN = BATCH_SIZE * 20 # Bắt đầu học khi có nhiều kinh nghiệm hơn
TARGET_UPDATE_FREQ = 10000

# Lưu trữ model
SAVE_MODEL_EVERY_EPISODES = 500
MODEL_SAVE_DIR = os.path.join(current_dir, "saved_models_rendered") # Thư mục lưu model
MODEL_FILENAME_PREFIX = "dqn_pacman_ghosts"
...
FPS_WHILE_RENDERING = 15 # FPS khi hiển thị game (chậm hơn để dễ quan sát)
# -----------------------------------------
...
def train():
    ...
    if LOAD_PRETRAINED_MODEL and os.path.exists(PRETRAINED_MODEL_PATH):
        ...
        agent.load_model(PRETRAINED_MODEL_PATH, continue_training=True) #công tắc trạng thái để là train 
        # Nếu tắt cái train đi thì nn.Dropout sẽ bị vô hiệu hóa: Lớp này được 
        # thiết kế để "tắt" ngẫu nhiên các neuron trong quá trình huấn luyện để chống overfitting.
        #agent.epsilon = EPSILON_START # Bắt đầu với epsilon thấp nếu tải model (cho cái 20x22 mới làm)
    else:
