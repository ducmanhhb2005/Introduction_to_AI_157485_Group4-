from Overall.overall import DDX, isValid, isValid2, best_move 
...
def evaluationFunction(_map, _ghost, pac_row, pac_col, N, M, score):
    ...
    for (row, col) in _food_pos:
        distancesToFoodList.append(dis[(row, col), (pac_row, pac_col)])
    ...
    for [g_r, g_c] in _ghost:
        distance = dis[(pac_row, pac_col), (g_r, g_c)]
        if distance > 0:
...
def ExpectAgent(_map, _dis, prev_row, prev_col, pac_row, pac_col, N, M, depth, Score):
    def terminal(_map, _pac_row, _pac_col, _depth) -> bool:
    ...
    def _expect(_map, _ghost, _pac_row, _pac_col, _N, _M, _depth, score, agent):
        if terminal(_map, _pac_row, _pac_col, _depth):
