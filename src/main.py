...
from SearchAlgorithmsxDRL.Ghost_Move import *
from SearchAlgorithmsxDRL.SearchAgent import SearchAgent
from Object.Food import Food
from Object.Player import Player
from Object.Wall import Wall
from Overall.overall import DDX, isValid2
...
def initData() -> None:
    global N, M, _map, _food_position, _food, _road, _wall, _ghost, _visited, Score, _state_PacMan, _ghost_Position, _old_direction, _dis
    N = M = Score = _state_PacMan = 0
    _map = []
    _wall = []
    _road = []
    ...
    _dis = compute_all_pairs_shortest_paths(_map, N, M)
...
def startGame(level, algorithm, map_name, is_test = False, is_render = True):
    global _map, _visited, Score, Level, Algorithm, Map_name, _dis
    Level = level
    Algorithm = algorithm
    Map_name = map_name
    _ghost_position = []
        ...
        if delay <= 0:
            if is_moving:
                ...
                if len(_ghost_new_position) > 0:
                    _ghost_position = _ghost_new_position
                    for idx in range(len(_ghost)):
                ...
                search = SearchAgent(_map, _food_position, _ghost_position, _dis, prev_row, prev_col, row, col, N, M)
...
def handleEndGame(status: int):
    ...
    while not done_2:
        ...
        if status == -1:
            screen.blit(bg, (0, 0))
            # text_surface = my_font_2.render('Your Score: {Score}'.format(Score=Score), False, YELLOW)
