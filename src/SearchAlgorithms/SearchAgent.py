...
class SearchAgent:
    def __init__(self, _map, _food_Position, prev_row, prev_col, start_row, start_col, N, M):
        self.map = _map.copy()
    ...
    def execute(self, ALGORITHMS, visited=None, depth=4, Score=0):
        ...
        if ALGORITHMS == "Minimax":
            return minimaxAgent(self.map, self.prev_row, self.prev_col, self.start_row, self.start_col, self.N, self.M, depth, Score)
